...
precision = '/export/jw/cnn/data/acc.lstm'
...
def build_vocab():
    ...
    vocab['UNKNOWN'] = code
    code += 1
    for line in open('/export/jw/cnn/data/train'):
        ...
        for i in range(2, 3):
            ...
            for word in words:
                if not word in vocab:
                    vocab[word] = code
                    code += 1
    for line in open('/export/jw/cnn/data/test1'):
        items = line.strip().split(' ')
        for i in range(2, 3):
            words = items[i].split('_')
            for word in words:
...
def load_vectors():
    vectors = {}
    for line in open('/export/jw/cnn/data/vectors.nobin'):
        items = line.strip().split(' ')
        if (len(items) < 101):
            continue
        vec = []
